...
SDK 的 `extra_headers={"session_id": "稳定对话标识"}` 是实际 HTTP 请求头，与原生 Codex
metadata 中的 `session_id` 使用同一个绑定命名空间。显式 `session_id` 头优先于正文中的
`thread_id` / `conversation_id` 等备用标识；正文或 turn metadata 已选出的 `session_id`
与该头不一致时返回 400。没有该头时沿用现有 metadata 优先级和自动续接匹配。
同一任务的各轮、工具续接、压缩和恢复应复用该标识；独立任务使用不同值。队列仍按可信调用方
隔离绑定，出站时将客户端标识替换成服务端身份。请求 ID、SDK 运行环境头和缓存键不作为会话 ID。
